...
export class AuthorizeService {
  ...
  private get getPermissions(): any[] {
    ...
    //   this.getUser();
    //   if(!(!!this.userSubject.value.permissions)) return [];
    // }
    if(!this.userSubject.value || !this.userSubject.value.permissions) return [];
    const permissions = this.userSubject.value.permissions;
    if(Array.isArray(permissions))  return permissions; 
    else  return [permissions];
  }

  public hasPermission(per: string | string[]) : boolean {
    var permission = [];
